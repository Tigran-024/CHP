...
std::atomic<bool> const serverRunning{ true };
static int WSAInit() 
{
...
int static SendDatagram(SOCKET s, char* msg, unsigned int msglen, struct sockaddr* si_dest, unsigned int slen) 
{
...
int static RecvDatagram(SOCKET s, char* buf, unsigned int buflen, struct sockaddr* si_dest, int* slen) 
{
...
int main(int argc, char* argv[]) 
{
    ...
    if (argc > 1 && strcmp(argv[1], "server") == 0) 
        server = std::thread(ServerThread);
